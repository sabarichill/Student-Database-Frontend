...
function StudentTable({ students, onEdit, onDelete, onSort, sortBy, sortOrder }) {
        ...
        <tbody>
          {students.map((s, i) => (
            <tr key={s.id}>
              ...
              <td>{s.course}</td>
              <td>
                <span className={`cgpa ${s.cgpa >= 8 ? 'high' : s.cgpa >= 6 ? 'mid' : 'low'}`}>
                  {s.cgpa}
                </span>
              </td>
              <td>{s.yop}</td>
              ...
              <td>
                ...
                <button onClick={() => onDelete(s.id, s.name)} className="btn-delete">🗑️ Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
